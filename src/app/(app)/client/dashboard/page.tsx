...
import {
    ...
    CalendarDays, Pencil, AlertTriangle, BookmarkCheck, Settings2,
} from "lucide-react";
...
function DayPanel({
    ...
    vacationRecord: VacationDoc | null; // null = not on vacation
    products: ProductDoc[];
    userDoc: { uid: string; defaultProduct?: string; defaultQty?: number } | null;
    ...
    const handleDispute = async () => {
        ...
        const qty_num = parseFloat(disputeQty.replace("L", ""));
        await onDispute(delivery.id, qty_num, disputeNote);
    ...
    const handleSetDefault = async () => {
        ...
        const qty_num = parseFloat(qty.replace("L", ""));
                ...
                <div className="bg-blue-950 border-t border-white/10 rounded-t-3xl p-6 pb-28 max-w-lg mx-auto shadow-2xl space-y-5">
                            ...
                            {vacation ? (
                                <div className="space-y-1">
                                    <span className="inline-flex items-center gap-1 text-xs text-purple-300 bg-purple-500/15 border border-purple-500/25 px-2 py-0.5 rounded-full">
                                        <PlaneLanding className="w-3 h-3" /> On Vacation
                                    </span>
                                    {vacationRecord && (
                                ...
                                <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full border ${isFutureDay
                                    ? "text-amber-300 bg-amber-500/15 border-amber-500/25"
                                    : "text-green-300 bg-green-500/15 border-green-500/25"
                                    }`}>
                                    <CheckCircle className="w-3 h-3" />
                                    {delivery.quantity.toFixed(1)}L {delivery.product_name} {isFutureDay ? "requested" : "delivered"}
                    ...
                    {!vacation && (
                        <div className="space-y-3">
                            <p className="text-white/50 text-xs uppercase tracking-widest flex items-center gap-1.5">
                                ...
                                {isFutureDay ? "Request for this day" : "Log / Update entry"}
                            </p>
                            <div>
                                ...
                                >
                                    {saved ? <><CheckCircle className="w-4 h-4" /> Saved!</> :
                                        saving ? <div className="w-4 h-4 border-2 border-white/40 border-t-white rounded-full animate-spin" /> :
                                            <><Milk className="w-4 h-4" />{delivery ? "Update" : "Save Entry"}</>}
                                </button>
                    ...
                    {vacation && vacationRecord && (
                        <div className="bg-purple-500/10 border border-purple-500/20 rounded-2xl p-4 space-y-3">
                            ...
                            >
                                <X className="w-4 h-4" /> Cancel This Vacation
                            </button>
                        </div>
                    )}
                        ...
                        <div className="border-t border-white/10 pt-4">
                            {!showDispute ? (
                                ...
                                <div className="space-y-3">
                                    ...
                                    <ChipSelector options={QTY_OPTIONS} selected={disputeQty} onChange={setDisputeQty} />
                                    <textarea
                                        value={disputeNote}
...
export default function ClientDashboardPage() {
    ...
    const monthTotal = deliveries.reduce((sum, d) => sum + d.total_cost, 0);
    const monthLitres = deliveries.reduce((sum, d) => sum + d.quantity, 0);
    ...
    // Show current default in the header hint
    const defaultTypeName = userDoc?.defaultProduct
        ? (products.find((p) => p.id === userDoc.defaultProduct)?.name ?? "Buffalo")
        : "Buffalo";
    ...
    return (
        <div className="space-y-4">
                ...
                <div className="flex items-end justify-between">
                    <div>
                        ...
                        <p className="text-blue-300 text-sm mt-0.5 flex items-center gap-1">
                            <Milk className="w-3.5 h-3.5" /> {monthLitres.toFixed(1)}L delivered
                        </p>
                    </div>
                    <div className="text-right">
                        <TrendingUp className="w-8 h-8 text-blue-400/40 ml-auto mb-1" />
                        <p className="text-white/30 text-xs">Default: {defaultQty}L {defaultTypeName}</p>
                    </div>
                </div>
            </div>
            ...
            <p className="text-center text-white/25 text-xs">
                Tap any date to log, request, or dispute · Tap "Set Default" to save daily preference
            </p>
